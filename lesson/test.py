...
def get_data(path):
    ...
    # Create new features
    #第三 Polynomials on the top 10 existing features
...
X_dev,y_dev = get_data('./data/test.csv')
...
#y_dev_pred = np.exp(y_dev_pred)-1
print(y_dev_pred)
index = np.arange(1461,1461+y_dev_pred.shape[0])
index = np.c_[index,y_dev_pred]
index = pd.DataFrame(index,columns=['Id','SalePrice'])
index.to_csv('./result.csv',index=False)
